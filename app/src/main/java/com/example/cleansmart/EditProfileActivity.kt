...
class EditProfileActivity : Activity() {
    ...
    private fun validateInputs(): Boolean {
        ...
        if (phone.isNotEmpty() && !phone.matches(Regex("^[0-9+]{10,15}$"))) {
            phoneEditText.error = "Please enter a valid phone number"
            return false
        }

        // Username validation (no spaces or special chars except _ and .)
        val username = usernameEditText.text.toString()
        if (username.isNotEmpty() && !username.matches(Regex("^[a-zA-Z0-9_.]+$"))) {
            usernameEditText.error = "Username can only contain letters, numbers, underscores, and periods"
            return false
        }
    ...
    private fun saveProfileChanges() {
        ...
        profileData.email = emailEditText.text.toString()
        profileData.username = usernameEditText.text.toString()
        profileData.phone = phoneEditText.text.toString()
        profileData.address = addressEditText.text.toString()
        profileData.stylePreference = styleSpinner.selectedItem.toString()
        ...
        // Save other data to SharedPreferences
        ...
        with(sharedPrefs.edit()) {
            ...
            selectedImageUri?.toString()?.let { uriString ->
                putString("profileImageUri", uriString)
            }
            apply()
        }

        // Prepare intent with updated data
        val profileDataIntent = Intent(this, ProfileUpdateSuccessActivity::class.java).apply {
        ...
        startActivityForResult(profileDataIntent, PROFILE_UPDATE_SUCCESS_REQUEST)
    }
}